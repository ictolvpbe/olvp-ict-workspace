...
**FASE 1 IN UITVOERING (gepauzeerd 2026-06-12):**
- ✅ Proxmox-clone gedaan: `srvv-unifi-01` draait op **`10.35.0.15`** (user koos eerst `.70`, daarna ge-re-IP'd naar `.15` zoals gepland). ping + SSH-poort 22 open vanaf werkstation.
- ✅ Inventory-entry `unifi_servers > srvv-unifi-01` toegevoegd aan `platform-ansible/inventory.yml` — **NOG NIET gecommit** (bewust, tot baseline slaagt).
- ⚠️ AD-DNS-record `SRVV-UNIFI-01.olvp.int → 10.35.0.15`: status onbekend — resolvt nog NXDOMAIN vanaf werkstation; bevestigen of het record bestaat. `unifi.olvp.int` blijft met rust (cut-over-hefboom).
- ❌ **BLOKKER baseline = SSH-auth**: dit werkstation **`LPT-SCHK` (`/home/kristof/`)** mist de canonical ansible-key. Geladen key uit KeePassXC (`~/.ssh/ansible_olvp`, fp `SHA256:DpQLouNSmge8uyhCapNKeOoD51pOpsiDmrIgNbuq3Zg`) identificeert zich als **`ansible-service-account`** en wordt door `srvv-unifi-01` GEWEIGERD. Memory [[feedback-semaphore-key-canonical]]: de VM's vertrouwen **`ansible-targets-ssh`**, niet `ansible-service-account` → vermoedelijk verkeerde KeePassXC-entry geëxporteerd. **VOLGENDE STAP**: via Proxmox-console van srvv-unifi-01 `sudo ssh-keygen -lf /home/ansible/.ssh/authorized_keys` → fingerprint vergelijken; matcht niet → juiste key (`ansible-targets-ssh`-entry, private key zit mogelijk als attachment) exporteren + `ssh-add`. Daarna: baseline `ansible-playbook -i inventory.yml tier1-baseline.yml --limit srvv-unifi-01 --ask-vault-pass` (vault-pw interactief → user draait zelf met `! …`), post-verify Podman ≥4.3.1 + slirp4netns ≥1.2.
- Opruimactie blijft openstaan: fout root-bestand `~/.ssh/ansible-targets` (publieke key, root-owned) → `sudo rm`. Private key staat ook nog in `~/Downloads/ansible_olvp(.pub)` → verwijderen na correcte install (gevoelig).
