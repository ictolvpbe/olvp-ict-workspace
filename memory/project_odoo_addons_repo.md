...
**Voorstel 3-tier branch-model (2026-06-10, na het myschool-test-503-incident — [[project-odoo-role-migration]]):** introduceer een aparte **`test`-branch** zodat de **testserver niet langer rauwe `Dev` volgt**. Dat was dé oorzaak van het incident: elke half-af Dev-commit (bv. een nieuwe `sap_sync`-feature met nieuwe deps/velden) landde meteen op myschool-test. Nieuw model: **`Dev`** = dev (mag breken, myschool-dev/-dev2) → **`test`** = myschool-test (bewust **gepromote**, stabiel voor de testgroep) → **`master`** = prod. Promotie via merge `Dev`→`test`→`master`. **Implementatie**: ✅ **branch `test` aangemaakt + gepusht 2026-06-10** (afgesplitst van `origin/Dev`-tip `e640d89`, upstream `origin/test`). Resteert (bij de role-migratie): `instances.yml addons_branch: Dev`→**`test`** voor myschool-test + Semaphore-addons-update channel test → branch `test`. ⚠️ `test` = Dev-snapshot incl. de verse auth_oidc blok A/B — **vóór de eerste deploy naar de testgroep verifiëren dat dit een stabiele basis is** (anders `test` re-pointen naar een eerdere commit; pas net aangemaakt). Optioneel: GitHub branch-protection op `test` (enkel via merge/PR) om de promotie-discipline af te dwingen. Case-sensitief (kies `test` vs `Test` bewust, houd consistent met `Dev`/`master`). **Wordt mee geregeld bij de role-migratie ([[RB-2026-TST-ROLE-MIGRATE]] fase 3)**. **Cross-werf**: branch-aanmaak + promotie-discipline hoort bij de **odoo-dev-werf** (MySchool_addons, `/home/demm/PyCharm/odoo-myschool/extra-addons/`); de infra-kant zet enkel `addons_branch`. Te bevestigen met collega/odoo-dev-werf.
