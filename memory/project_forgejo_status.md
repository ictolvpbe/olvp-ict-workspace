...
- **Remote HTTP-variant OOK gebouwd 2026-06-28** (naast stdio): gedeeld endpoint voor headless/andere clients (Semaphore, collega-machine, cron). Architectuur:
  - Container **`forgejo-mcp`** (Quadlet `/etc/containers/systemd/forgejo-mcp.container`), image **`localhost/gitea-mcp:1.3.0`** lokaal gebouwd op de VM (Containerfile FROM debian:13-slim + officiële binary; geen OCI-image upstream). Op `forgejo-net`, praat intern met **`http://forgejo:3000`** (geen TLS/DNS-gedoe), luistert `127.0.0.1:8082`. Transport = **http** (streamable, endpoint-pad `/mcp`), NIET sse.
  - **Endpoint**: `https://git.olvp.int/mcp` via Caddy path-based route (`@mcp path /mcp /mcp/*` → bearer-check → `reverse_proxy 127.0.0.1:8082`; rest → Forgejo). Bearer beschermt het endpoint want gitea-mcp houdt zelf het Forgejo-token; wie het endpoint bereikt handelt als de service-user.
  - **Identiteit**: aparte user **`claude-mcp-remote`** (id 2... eigenlijk aparte id, non-admin) + token-name `claude-mcp-remote-http`, zelfde scopes. Bewuste audit-scheiding workstation (claude-mcp/stdio) vs gedeeld endpoint (claude-mcp-remote/http). Token via Podman-secret `gitea-mcp-remote-token` (type=env→GITEA_ACCESS_TOKEN).
  - **Token-in-image-secret**: gitea-mcp ondersteunt `GITEA_ACCESS_TOKEN` / `GITEA_ACCESS_TOKEN_FILE` env. Geverifieerd: met bearer→200, zonder→401, Forgejo-UI→200; `claude mcp add --transport http forgejo-remote https://git.olvp.int/mcp -H "Authorization: Bearer <bearer>"` → ✔ Connected.
  - **Credentials in KeePassXC** (`olvp-forgejo-claude-mcp-remote`): service-user-pw, Forgejo-token, **én de Caddy-endpoint-bearer**. NOOIT in git.
  - **Op het hoofd-werkstation NIET geregistreerd gelaten** (stdio volstaat lokaal; dubbele registratie = dubbele tools). Remote is bedoeld voor ANDERE clients.
  - **Codificatie**: container + Caddy-route in `forgejo.yml`/`forgejo.Caddyfile.j2` (zie commit); secrets verwacht in vault (`forgejo_mcp_remote_token`, `forgejo_mcp_bearer`) — handmatig toe te voegen want vault-edit vereist vault-pw. Playbook faalt fail-fast als ze ontbreken. Image-build is een gedocumenteerde manuele prereq (fail-fast gate), net als step-CLI/cert.
- Gerelateerd: [[project-mcp-and-itsm]].

**INCIDENT `/var` 100% vol 2026-06-28 (opgelost)**: tijdens de remote-MCP-build faalde `podman build` met `no space left on device`. Oorzaak: `/var/log/{syslog,user.log}` ~9,6G, gevuld door **Podman health_status-events** (postgres/forgejo healthchecks elke 30s) die rsyslog dubbel wegschreef; logrotate draaide dagelijks i.p.v. op grootte → ongebreidelde groei. NIET Forgejo-data, NIET podman-images (~270M reclaimable). Fix: (1) `truncate -s 0` syslog+user.log + rm .1 → ~9,6G terug; (2) `lvextend -l +100%FREE /dev/SRVV-DEBIAN-TEMPL-vg/var` + `resize2fs` → 12G→**19G** (7G VFree was beschikbaar); (3) root-cause = rsyslog drop-filter `/etc/rsyslog.d/49-drop-podman-health.conf` (`if ($programname == "podman") and ($msg contains "health_status") then stop`) — 0 nieuwe spam-regels na restart; (4) logrotate `maxsize 200M` backstop in `/etc/logrotate.d/rsyslog`. HealthInterval bewust ongemoeid (rsyslog-filter lost bron al op, geen postgres-restart nodig). LES: container-hosts met Podman-healthchecks → rsyslog health_status-spam is een sluipende /var-vuller; check dit patroon op andere VMs (TST/PROD-webapps draaien ook Podman-healthchecks).

**Cert OPGELOST 2026-06-28**: vers step-ca cert voor git.olvp.int uitgegeven (de eerder verlopen 24u-cert van 2-jun). Bewijs: `curl` ZONDER `-k` vanaf werkstation → HTTP/2 200 (geldig + getrust cert; verlopen cert zou falen). Werkstation trust step-ca-root al (geen browser-cert-warning meer — oude memory-notitie daarover achterhaald).

**Renewal end-to-end GEVERIFIEERD 2026-06-28**: timer-service `caddy-cert-renew` no-opt correct bij >12u (`certificate not renewed: expires in 22h...`); force-renew (`step ca renew --force`) bewees het mechanisme — `notAfter` schoof vooruit ZONDER provisioner-pw (bestaand geldig cert = auth, via stepca.olvp.int:8443 + /root/.step/certs/root_ca.crt). Na renew + caddy-restart: UI 200 + MCP 200. Conclusie: handenvrij, de 24u-cert-renewal-saga keert niet terug.
