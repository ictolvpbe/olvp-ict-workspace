...
- `static/src/scss/login.scss` in `web.assets_frontend` → loginscherm (achtergrond + knop + links + focus; witte kaart). **Volgt de ingestelde menu-kleur**: de frontend-bundle kent de backend-`--myschool-*`-vars niet, dus `res.company._olvp_login_css_vars()` leest `brand_1` (via `read_theme_colors`) en injecteert afgeleide `--olvp-login`/`-dark`/`-ring` op de login-`<html>` (branding_templates.xml). login.scss gebruikt die met OLVP-petrol als fallback. (commit 05c7b67)
...
**Uitrol = normale intranet-deploy** (geen Ansible-role): push branch `dev` → `addons-update` doet `git pull` + `-u intranet_theme` op intranet-dev; test/acc/prod volgen via hun branches (acc-test/test = `test`, acc/prod = `prod`). Verifieer: Instellingen → OLVP-thema-tab; petrol `/web/login`; favicon in tab.

**Status 2026-06-27:** code klaar + gevalideerd (py/xml/manifest/migratie). **`dev` GEPUSHT** naar `ictolvpbe/intranet` (commits `81a0501` + `05c7b67`, samen met user-commits ldap/sync). **addons-update op intranet-dev moet de USER draaien** (vault + PAT zitten niet in agent-shell): Semaphore-template addons-update met survey `target_fqdn=intranet-dev.olvp.be`, `upgrade_modules=intranet_theme`, `restart_after=true` (VLAN207 → Semaphore bereikt het); of lokaal `ansible-playbook addons-update.yml -e target_fqdn=intranet-dev.olvp.be -e upgrade_modules=intranet_theme -e restart_after=true --ask-vault-pass` (met `export github_pat=...`). Nog te verifiëren na de run. Daarna test/acc/prod.
