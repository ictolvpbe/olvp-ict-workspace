...
**✅ Kiosk werkt 2026-08-27**: na M-004 volgde `Forbidden` — de Caddy-vhost laat alleen `internal_networks` door en VLAN 73 stond daar niet in. Bewust NIET aan `internal_networks` toegevoegd (die geeft ook toegang tot Odoo-beheerpaden) maar een aparte var **`netxms_ui_extra_networks`**. Eindstand: 70 verzoeken van 10.70.4.20, status 200 met actieve RAP-sessie.
**⚠️ Wachtwoord lekte in de Caddy-toegangslog**: de web-UI meldt zich aan via de **query-string**, dus het kiosk-wachtwoord stond bij élk verzoek in de log — die ook naar Loki gaat. Filter toegevoegd, maar let op: `request>uri query { delete password }` volstond NIET, want de browser stuurt de oorspronkelijke URL mee als **`Referer`**. Ook `Cookie` verwijderd. ⚠️ **Oude regels staan al in Loki** (retentie 90 dagen) → overweeg het `kiosk-tv`-wachtwoord te roteren.
**Playbook-lessen**: handler `update-ca-certificates` vuurt niet als een eerdere mislukte run het bestand al plaatste → expliciete controle toegevoegd; en die controle kan **niet** met een grep op `/etc/ssl/certs/ca-certificates.crt` (alleen PEM-blokken, geen leesbare namen) → controleren op de symlink `step-ca-root.pem`.

**SEC-5 → GEPLAND 2026-08-28**: wordt opgelost bij de indienststelling van de **UniFi Enterprise Fortress Gateway (EFG)**, uiterlijk **2026-09-10**. Zie [[project-gateway-cutover]] — dat is de volledige cutover met eigen checklist, dus SEC-5 schuift mee als die schuift.
**Oorspronkelijk (2026-08-27) aanvaard**: Webmin/Usermin/xrdp/postfix blijven staan. Reden: het toestel wordt **ook voor offline back-ups** gebruikt en staat in een afgesloten lokaal. Kanttekening vastgelegd: fysieke beveiliging dekt geen netwerkpad (diensten op `0.0.0.0`, bereikbaar vanuit VLAN 73); tussenweg = binden aan de beheerdersband. Herzien bij wijziging van de bereikbaarheid van VLAN 73. **`AUTOSTART/`-bestanden zijn verwijderd**, maar het `system`-wachtwoord blijft gecompromitteerd → roteren of meenemen in de decommissie van 10.10.100.2.
...
### 2026-09-21 — PVE-productiecluster op de nieuwe server
De drie productie-hypervisors (`srv-pmclust-p01/02/03`, 10.10.100.80-82, PVE 9.2 = trixie) **droegen al een agent**: versie **5.1.4**, en niet eens gelijk — p01 uit de `bookworm`-repo, p02/p03 uit `bullseye`. Allemaal met `MasterServers=10.10.100.2`, dus de uit te faseren server had remote command execution op de laag die élke gast draagt. `netxms.list` stond volledig uitgecommentarieerd (pakket `netxms-release` 1.13), dus ze zaten muurvast zonder updatepad.
Nu via `netxms-agent.yml` op **6.2.3-1+trixie**, `Servers=10.35.0.20` (alleen-lezen), pin + deb822-repo erop. Op de host geverifieerd, niet op de PLAY RECAP: dpkg-versie, `nxagentd -v`, service active, poort 4700, en `Servers=` zonder 10.10.100.2. Firewall-pad .20 → .80/.81/.82:4700 was al open (vooraf getest vanaf de monitoring-VM).
**`pve_prod` is nu kind van `netxms_agents`** — de oude uitsluiting ("PVE-mgmt-net niet bereikbaar vanaf de controller") klopte niet meer. `pve_test` blijft eruit: dat cluster staat standaard uit (DR-doel).
**Bewust géén legacy-server op de hypervisorlaag**: `netxms_legacy_server: ""` in `group_vars/pve_nodes.yml`. Parallel draaien met 10.10.100.2 geldt voor de VM's; de hypervisors gaan in één keer over.
**`netxms-release` gepurged** (1.13, zelfde dag): dat pakket zette een tweede repo-definitie neer (`netxms.list`) én een sleutel in `/etc/apt/trusted.gpg.d/` — systeembreed vertrouwd i.p.v. per repo zoals onze `Signed-By`. De regels stonden toevallig uitgecommentarieerd, maar een upstream-apt-ronde kan ze weer aanzetten en dan is de pin niet langer het enige woord. Geen reverse deps, dus schoon weg; `apt update` rc=0 en `apt-cache policy` houdt 6.2.3 vast terwijl 6.2.4/6.2.5 in de repo staan — de pin doet dus wat hij moet. **Gecodificeerd als taak in `netxms-agent.yml`** (tag `repo`), zodat elke andere agent-host hem ook kwijtraakt.

**⚠️ Precedentie-val (gekost: een halve uitrol)**: dezelfde override stond eerst als groepsvar in `inventory.yml` en werd stil genegeerd — `Servers=10.35.0.20,10.10.100.2` kwam er toch uit. Inventarisgroepsvars staan **lager** dan élk `group_vars`-bestand, ook dat van `all`. Alles wat `group_vars/all/vars.yml` ook zet (`ansible_ssh_common_args`, `netxms_legacy_server`) hoort dus in `group_vars/<groep>.yml` of per host — niet in de groeps-`vars:` van de inventaris. Zo deed de rest van de inventaris het al (per host), vandaar dat het niet eerder opviel.

### 2026-09-21 (2) — vloot nagekeken: uitrol was al klaar, repo-hygiëne niet
Bij het voorbereiden van "de rest van `netxms_agents`" bleek er **niets meer uit te rollen**: haproxy-1/2, tst/dev/srvv-odoo-01, srvv-acc-01, srvv-forgejo-01, srvv-id-01, srvv-monitoring-01, srvv-unifi-01, srvv-sspr-01 en srvv-p-backup-01 draaiden allémaal al 6.2.3-1+trixie met `Servers=10.35.0.20,10.10.100.2`. Vanaf .20 getest: poort 4700 open op alle elf remote adressen. Punt 3 van deze lijst stond dus onterecht nog open — alleen de hypervisors ontbraken.
Wat wél overal nog stond: **`netxms-release` 1.13**, op alle 12. Met `netxms-agent.yml --tags repo` gepurged (tag raakt enkel keyring/bron/pin/purge — geen herinstallatie, geen agent-herstart). Nagekeken op alle 15 hosts: pakket weg, `/etc/apt/trusted.gpg.d/` netxms-vrij, `netxms.list` weg, `netxms.sources` intact, agent 6.2.3 active, en de pin houdt 6.2.3 vast terwijl 6.2.4/6.2.5 in de repo staan.
**Gotcha bij verifiêren**: `apt-cache policy` volgt de locale — op deze hosts staat er *Kandidaat:*, niet *Candidate:*. Een grep/awk op "Candidate" geeft stil niets terug en lijkt op een kapotte repo. Parse met `LC_ALL=C apt-cache policy`.
Nog open: de **legacy-server 10.10.100.2** staat nog in de config van de 12 VM's (bewust, parallelle fase). Dat is punt 5 hieronder.

### 2026-09-21 (3) — link-up/down-ruis: classificatie via expectedState
Vraag: alleen alarmeren op poorten naar een ander asset (switch/AP/server), niet op poorten naar een pc. **Mechanisme = `expectedState` per interface**, niet de EPP-regel: UP→alarm bij wegvallen, DOWN→alarm bij onverwacht opkomen, IGNORE→geen event. De default-EPP-regels 4/5/7/8 blijven ongewijzigd; ze krijgen gewoon geen events meer.
**Meetstand**: 8277 interfaces, 4390 nu down. AP's zijn de grootste bron (4875 ifaces / 1789 down — UniFi exporteert elke kernel-interface: `lo`, `sit0`, `gre0`, `ifb0`, `soc0`, `wifi0ap7`…; alleen `eth0` telt). Switches 3126/2553, servers 276/48 (ongebruikte `eno7`/`eno8`, `docker0`, en Proxmox' `tap`/`vnet`/`fwbr` die bij elke VM-start komen en gaan).
**Twee signalen, geen van beide alleen voldoende**: (a) `peerNode` dekt maar 278 van de 1506 fysieke switchpoorten — UniFi levert nauwelijks LLDP (42 van 434), de rest is FDB-matching die een buur alleen ziet zolang die praat; (b) de naamconventie in `if_alias` mist kritieke toestellen die geen NetXMS-node zijn: `BRANDCENTRALE BLOK A`, `UPS-PL208-01`, `PROXIMUS MODEM`, `GW001-LAGG-ibg2_3`, `toegangscontroller F0-01`, `HVAC STOOKRUIMTE`. Vandaar een trefwoordenlijst áchter peer+snelheid(≥10G)+conventie, plus custom attribute `olvp.port` als handmatige override die elke run overleeft.
**Geleverd**: `platform-ansible/files/netxms/olvp-port-class.nxsl` (NXSL, dryRun-first) + runbook **RB-2026-NETXMS-PORTCLASS**. NXSL-API geverifieerd tegen `libnxcore.so.62` van de draaiende 6.2.3 (`SetInterfaceExpectedState`/`GetAllNodes`/`GetNodeInterfaces`/`isPhysicalPort`/`GetCustomAttribute` bestaan alle).
**Nog te doen door mens**: droogloop draaien in de console (Script Library → `OLVP::PortClass`) — controleer daarbij hoe `expectedState` zich toont (getal vs tekst) — en de **68 infra-gelabelde poorten die neer staan** nalopen; 17 daarvan zitten op `SW-PL208-C1/C2-BACKUP` (vermoedelijk reserve-switches die uit staan). Het script bevriest de huidige toestand, dus een poort die kapot is tijdens de run wordt vastgelegd als "hoort neer te zijn".
**DB-toegang**: read-only queries via `sudo podman exec -i netxms-db psql -U netxms -d netxms` op .20. Nooit schrijven terwijl netxmsd draait.

### 2026-09-22 — poort-classificatie TOEGEPAST
`OLVP::PortClass` draait en is toegepast: **6349 interfaces bekeken, 5526 gewijzigd** → UP 532 / DOWN 321 / **IGNORE 5496**. In de database geverifieerd (expected state = bits 28-29 van `object_properties.flags`; `0x20000000` = IGNORE): switch 414 UP / 994 IGNORE, overig 181 UP / 160 DOWN / 4502 IGNORE. Alle open `IF_UNEXPECTED_UP`- en `IF_DISABLED`-alarmen zijn vanzelf afgesloten door EPP-regels 6 en 9 (terminate-regels op `SYS_IF_EXPECTED_STATE_*`, severity 0/log-only). Open alarmen nu: 227, en **geen enkele link-gerelateerde** meer.
**Correcties op het ontwerp tijdens de droogloop**: eerste run gaf UP=2271 — veel te veel. UniFi-AP's exporteren ook `br0`/`br0.40`, `vwireap4`/`vwiresta5` (mesh) en `qca-nss-dev0-3` (offload); alleen `eth0` is de echte uplink. `PSEUDO_IF` uitgebreid met die namen + de generieke tak `.*\.[0-9]+` (elke VLAN-subinterface erft de linkstatus van zijn ouder, dus bewaak de ouder).
**NXSL 5-deprecations onderweg** (console geeft warnings, nxscript niet): `upper(s)`→`s->toUpperCase()`, `GetCustomAttribute(o,a)`→`o->getCustomAttribute(a)`, `GetNodeInterfaces(n)`→`n->interfaces`. `GetAllNodes()` blijft (geen alternatief in de binary). Zie [[feedback-nxsl5-concat]].
**Timing-val bij verifiêren**: objectwijzigingen staan pas na ~15 s in de DB. Een controle-query meteen na de run toont nog de oude vlaggen — dat lijkt op "het script deed niets".
**Nog open**: (1) scheduled task `Execute.Script`/`OLVP::PortClass` dagelijks; (2) `Objects.Interfaces.DefaultExpectedState` van AUTO naar IGNORE; (3) de 68 infra-poorten die neer staan nalopen (17 op SW-PL208-C1/C2-BACKUP); (4) 1620 niet-fysieke switch-interfaces (SVI's) staan nog op DOWN — script slaat ze over; (5) volgende ruisbron = `SYS_MAC_ADDR_CHANGED` (206 open, 1282 in 4 weken, EPP-regel 35).

### 2026-09-22 (2) — dekkingsgat: 33 nieuwe AP's onzichtbaar voor NetXMS
UniFi-inventaris (via `unpoller_device_info` in Prometheus, **geen controller-credentials nodig**) naast de NetXMS-nodes gelegd: **236 toestellen in UniFi (62 switches, 174 AP's), 195 daarvan in NetXMS, 41 niet.**
- **Switches: 62/62 gedekt.** Geen enkele ontbreekt.
- **33 AP's in 10.70.1.0/24 zijn online in UniFi maar staan niet in NetXMS** — en de monitoring-VM bereikt dat subnet niet (ICMP noch TCP22, terwijl AP's in 10.1.111.x wél antwoorden). Actieve discovery heeft `10.70.1.1-10.70.6.253` al in de adreslijst, dus het is **geen scope-probleem maar een firewall-/routeringsgat** VLAN 35 → 10.70.1.0/24.
- **Dit gat groeit**: de 10.70.1.x-toestellen zijn de nieuwe U6ENT-AP's, en in 10.1.111.x staan hun voorgangers met `-OUD` in de naam. Naarmate de AP-migratie vordert zakt de monitoringdekking stil weg.
- 8 overige ontbrekende AP's (10.1.111.x) zijn **ook in UniFi offline**; 5 dragen `-OUD` (uitfasering), 3 niet: WAP-PA0BUI-01, WAP-PD0G1-01, WAP-PN101-01 — die verdienen een controle.
- Omgekeerd 1 wees: `sw-pparking` (10.10.110.248) staat in NetXMS maar kent UniFi niet.
**Firewall-regel M-006 actief 2026-09-22**: eerste versie liet enkel ICMP door, na toevoegen van UDP 161 antwoordt **SNMP op alle 33 AP's (33/33)**. Discovery moet ze nu vanzelf oppikken; daarna classificeert `OLVP::PortClass` hun interfaces in de nachtrun. Vastgelegd in `firewall-rules-matrix.md` als **M-006**.
Lijst: `unifi-netxms-dekking.txt` in de werkmap. Methode herbruikbaar: `curl 127.0.0.1:9091/api/v1/query?query=unpoller_device_info` op .20 (Prometheus luistert op **9091**, want 9090 is Cockpit).

### 2026-09-22 (4) — EFG stuurt nu wél syslog, en dat is de securitystroom
User zette de syslogserver op **10.35.0.1** naar .20. Sindsdien stuurt de EFG zelf CEF met **`UNIFIcategory=Security`** — precies de categorie die in de controller ontbreekt. Dominant: event **203 "Blocked by Firewall"**, **22,6 berichten/minuut (~32.000/dag)**.
**Twee vondsten die het ontwerp bijstuurden**:
1. **`CEF:` zit niet in msg_text bij de EFG.** Bij de controller staat `CEF:0|Ubiquiti|...` in `msg_text`; bij de gateway splitst de syslogparser `CEF` af als **msg_tag** en begint `msg_text` bij `0|Ubiquiti|...`. Mijn regexes waren op `CEF:` geankerd en misten dus precies de firewall-/IDS-events. Nu `(?:CEF:)?0\|Ubiquiti` — getest: 40/40 match.
2. **Splitsen op policy-naam, niet op event-ID.** Bijna alle 203-events zijn policy `Block All` op verkeer náár de gateway (poort 7680 Windows Delivery Optimization, HTTP, IPP, SMB) = ruis → apart event `OLVP_UNIFI_FW_BLOCK`, **flags=0 (niet gelogd), geen EPP-regel**. Wat onder een andere policy valt — er zat er één tussen onder **`Scanning Activity`** (poort 22) — blijft in de securityklasse. Op 40 berichten: 39 ruis, 1 signaal.
**Valkuil met volgorde (kostte een ronde)**: parser géplakt vóór de events bestonden → NetXMS bewaart de regels maar **gooit de `<event>`-verwijzing stilzwijgend weg**. Acht regels, drie met event. Geen foutmelding. Controle: `select var_value from config_clob where var_name='SyslogParser'` en kijken of elke rule een `<event>` heeft.
**Nog open**: EFG-berichten worden aan **`git.olvp.int`** gekoppeld i.p.v. aan de gateway-node (zelfde misbinding als bij 163 CEF-berichten eerder; DNS klopt: `SRV-P-EFG-01` → 10.10.0.1, een interface van de node `unifi`). Alarmen landen daardoor op de Forgejo-server. Uitzoeken: `Syslog.NodeMatchingPolicy=1` + resolver-cache.
**Adressering uitgeklaard**: 10.10.0.1 en 10.35.0.1 = dezelfde **EFG** (UniFi OS, `CN=unifi.local`, node `unifi`, één poot per VLAN). De **Network-controller** draait op **10.10.100.15:8443** (`/status` → server_version 10.0.162, gelijk aan de CEF-versie; unpoller haalt daar ook zijn 236 toestellen op). SRVV-UNIFI-01 (10.35.0.15, poort 11443) is de migratiedoelhost, daar draait het niet.
**Nevenvondst**: syslogvolume is enorm — SW-PN003-01 alleen al 328.331 berichten in 7 dagen (~47k/dag), totaal ~543k/dag. Kandidaat voor een rsyslog-filter zoals bij de wifi-kicks.

### 2026-09-22 (3) — UniFi-alarmen: de data lag er al, de parser ontbrak
Vraag: kritieke alerts uit de EFG (VPN, IDS). Bevinding: **de controller stuurt al CEF-events naar syslog** — 3400 in 30 dagen (113/dag), al opgeslagen in de NetXMS-syslogtabel. Er gebeurde niets mee: de syslogparser bevatte enkel de drie **MikroTik**-standaardregels (hardware die hier niet staat). 16,3 miljoen syslogregels, nul events.
**Geleverd**: `platform-ansible/files/netxms/olvp-syslog-parser.xml` (vier klassen i.p.v. een regel per event-ID, zodat toekomstige event-ID's vanzelf meekomen) + runbook **RB-2026-NETXMS-UNIFI**. Getoetst door de regexes los te laten op 3400 echte berichten: CRITICAL 264 (8,8/dag), RECOVERED 86, ADMIN 45, SECURITY 30 — en 3061 client-events (400-404) bewust genegeerd. Device Reconnected (513) sluit via een terminate-regel het offline-alarm; alarm-key bevat de toestelnaam (`%4`, optionele capture) zodat twee offline toestellen twee alarmen geven.
**⚠️ Voorwaarde**: de CEF-berichten komen van hostnamen die resolven naar **10.10.100.15** (de host waar de controller feitelijk draait) en **daarvoor bestaat geen node in NetXMS** — 3237 van 3400 berichten hangen aan `source_object_id=0`, 163 zijn ooit verkeerd aan git.olvp.int geplakt (resolver-cache). Zonder die node landen de alarmen nergens. De productie-controller wordt dus vandaag helemaal niet bewaakt (SRVV-UNIFI-01 10.35.0.15 bestaat wél als node, maar daar draait het niet).
**Niet aanwezig**: IDS/IPS- en VPN-events. Alleen de categorieën **System** en **Monitoring** staan aan in de controller; *Security Detections* moet nog aan (én Threat Management op de EFG). De parser is er al op voorbereid (`UNIFIcategory=Security`). VPN via metrics is een doodlopend spoor: `unpoller_site_remote_user_active` staat op 0 en `unpoller_site_gateways` ook — de EFG komt in unpoller's inventaris niet voor (enkel uap/usw).
**Nevenvondsten**: `Syslog.RetentionTime`=30 dagen (incidentonderzoek kijkt niet verder terug); **SW-PR1B1-01 en SW-PP106-01 lopen 4-5 dagen vóór op de klok** (NTP), wat elke tijdsgebonden drempel verpest; en er staat een U6 Enterprise *Ready for Adoption* in de controller.
...
3. ~~`netxms-agent.yml` uitrollen~~ **✅ KLAAR 2026-09-21**: 15 hosts dragen 6.2.3-1+trixie, service active, poort 4700 open vanaf .20. De 12 VM's op `.20 + legacy`, de 3 PVE-prod-nodes enkel op `.20`.
