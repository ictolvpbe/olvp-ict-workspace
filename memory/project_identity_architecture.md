...
**RESTEERT = auth_oidc Odoo-kant (B), odoo-dev-werf**: OCA `auth_oidc` vendoren in MySchool_addons + installeren; OIDC-provider (code-flow, discovery, client_id `odoo-myschool-dev2` + secret); `groups`-claim → Odoo-groep-mapping (auth_oidc-basis of custom); **internal user** i.p.v. portal; **login=email + geen JIT/link-bestaande + sAMAccountName/employeeID-fallback** (beslist 2026-06-09, zie update onderaan). Infra (back-channel/AddHost/REQUESTS_CA_BUNDLE/firewall) staat klaar. **Handoff-spec voor de extra-addons-sessie: `platform-handbook/hosting/operations/auth-oidc-odoo-spec.md` (SPEC-2026-AUTH-OIDC-ODOO).** NB: acc-instances zijn GEEN prereq (die doen direct-LDAP, niet OIDC); auth_oidc is testbaar op myschool-dev2 met bestaande data.
